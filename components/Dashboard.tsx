...
const KPICard = ({ title, value, subvalue, icon: Icon, colorClass, alert = false }: any) => (
  <div className={`bg-white rounded-xl p-4 shadow-sm border border-slate-100 flex flex-col justify-between hover:shadow-md transition-shadow ${alert ? 'ring-2 ring-rose-500 bg-rose-50' : ''}`}>
    <div className="flex justify-between items-start mb-2 gap-2">
      <div className="text-slate-500 text-xs font-medium uppercase tracking-wide truncate flex-1">{title}</div>
      <div className={`p-1.5 rounded-lg ${colorClass} bg-opacity-10 flex-shrink-0`}>
        <Icon className={`w-4 h-4 ${colorClass.replace('bg-', 'text-')}`} />
      </div>
    </div>
    <div>
      <div className={`text-xl lg:text-2xl font-bold truncate ${alert ? 'text-rose-600' : 'text-slate-800'}`} title={value}>
        {value}
      </div>
      {subvalue && <div className="text-xs text-slate-400 mt-1 truncate" title={subvalue}>{subvalue}</div>}
    </div>
  </div>
);
...
export const Dashboard: React.FC<DashboardProps> = ({ state }) => {
  ...
  const calculateRatio = (val: number) => {
    if (metrics.grossProfit <= 0) return '0.0';
    return ((val / metrics.grossProfit) * 100).toFixed(1);
  };
    ...
    <div className="flex-1 h-full overflow-y-auto bg-slate-50 p-4 lg:p-8 pb-24 lg:pb-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        ...
        <KPICard 
          ...
          subvalue={`利益率: ${metrics.totalRevenue > 0 ? ((metrics.operatingProfit / metrics.totalRevenue) * 100).toFixed(1) : '0.0'}%`}
          ...
          <div className="h-64 flex-1">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={structureData} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
                ...
                <YAxis tickFormatter={(val) => `¥${val/10000}万`} width={60} tick={{fontSize: 12}} />
          ...
          <div className="h-64 flex-1">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={breakEvenData} margin={{ top: 5, right: 30, left: 10, bottom: 5 }}>
                ...
                <XAxis 
                  dataKey="rate" 
                  label={{ value: '反響率 (%)', position: 'insideBottomRight', offset: -5, fontSize: 12 }} 
                  type="number"
                ...
                <ReferenceLine x={state.marketing.responseRate} stroke="red" strokeDasharray="3 3" label={{ value: "現在値", fontSize: 10, fill: "red" }} />
        ...
        <div className="flex flex-col lg:flex-row items-center justify-around gap-8">
           ...
           <div className="w-full lg:w-1/2 space-y-3">
              <div className="p-3 bg-emerald-50 rounded-lg border border-emerald-100">
                <div className="text-xs text-emerald-800 font-bold mb-1">ドアノック商品 (換気扇)</div>
                <div className="flex flex-col sm:flex-row sm:justify-between sm:items-baseline gap-1">
                   <span className="text-sm font-medium text-slate-700">利益: {formatCurrency(metrics.profitFan)}</span>
                   <span className="text-xs sm:text-sm bg-emerald-100 text-emerald-700 px-2 py-0.5 rounded-full inline-block w-fit">
                     構成比 {calculateRatio(metrics.profitFan)}%
                   </span>
                </div>
              </div>
              <div className="p-3 bg-blue-50 rounded-lg border border-blue-100">
                <div className="text-xs text-blue-800 font-bold mb-1">アップセル (コンロ)</div>
                <div className="flex flex-col sm:flex-row sm:justify-between sm:items-baseline gap-1">
                   <span className="text-sm font-medium text-slate-700">利益: {formatCurrency(metrics.profitStove)}</span>
                   <span className="text-xs sm:text-sm bg-blue-100 text-blue-700 px-2 py-0.5 rounded-full inline-block w-fit">
                     構成比 {calculateRatio(metrics.profitStove)}%
                   </span>
                </div>
              </div>
              <div className="p-3 bg-purple-50 rounded-lg border border-purple-100">
                <div className="text-xs text-purple-800 font-bold mb-1">クロスセル (キッチン)</div>
                <div className="flex flex-col sm:flex-row sm:justify-between sm:items-baseline gap-1">
                   <span className="text-sm font-medium text-slate-700">利益: {formatCurrency(metrics.profitKitchen)}</span>
                   <span className="text-xs sm:text-sm bg-purple-100 text-purple-700 px-2 py-0.5 rounded-full inline-block w-fit">
                     構成比 {calculateRatio(metrics.profitKitchen)}%
                   </span>
                </div>
              </div>
           </div>
